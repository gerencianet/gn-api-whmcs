...
class GerencianetIntegration
{
	...
	public function testIntegration()
	{
		...
		curl_setopt_array($curl, array(
			CURLOPT_URL => "https://$environment.gerencianet.com.br/v1/authorize", // Rota base, homologação ou produção
	...
	public function create_charge($items, $invoiceId = null, $notification_url = '', $shipping = null)
	{

		$options = GerencianetIntegration::get_gn_api_credentials();
		$metadata = array(
			'custom_id' => strval($invoiceId),
	...
	public function pay_billet($charge_id, $expirationDate, $customer, $message, $fine, $interest, $discount = false)
	{
		...
		try {
			$api = new Gerencianet($options);
			$charge = $api->definePayMethod($params, $body);
	...
	public function pay_credit($charge_id, $customer, $message, $params, $discountValue )
	{
		...
		$options = GerencianetIntegration::get_gn_api_credentials();
		$chargeId = array('id' => $charge_id);
		$cep = str_replace('.', '', str_replace('-', '', $params['paramsCartao']['cep']));
		$billingAddress = [
			'street' => $params['paramsCartao']['rua'],
			'number' => (int)$params['paramsCartao']['numero'],
			'neighborhood' =>  $params['paramsCartao']['bairro'],
			'zipcode' => (string) $cep,
	...
	public function getPaymentMethod($charge_id)
	{
		$options = GerencianetIntegration::get_gn_api_credentials();
	...
	public function getErrorMessage($error_code, $property)
	{
		...
		switch ($error_code) {
			...
			case 4600035:
				...
				break;
			case 4600037:
				$message = 'O valor da emissão é superior ao limite operacional da conta. Por favor, solicite que o recebedor entre em contato com o suporte Efí.';
				break;
			case 4600073:
			...
			case 4600209:
				...
				break;
			case 4600210:
				$message = 'não é possível emitir três emissões idênticas. Por favor, entre em contato com nosso suporte para orientações sobre o uso correto dos serviços Efí.';
				break;
			case 4600212:
